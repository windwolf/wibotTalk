...
// static inline token_type _tree_accessor_next_token(const char *path,
//                                                    char const **token,
//                                                    char const **end,
//                                                    int32_t *size);
// static inline bool _tree_accessor_ctx_update(TreeAccessorContext *ctx,
//                                              const char *identity,
//                                              int identitySize, int index);

// static bool _tree_accessor_evaluate(TreeAccessor *tree, char *in,
//                                     char *outBuffer, ConsoleAction action);
// static bool _tree_accessor_path_parse(TreeAccessor *tree, const char *path);
// static TreeAccessorItemEntry *
// _tree_accessor_entry_child_find(TreeAccessorItemEntry *entry,
//                                 const char *identity, int size);
// static int _tree_accessor_match_name(const TreeAccessorItemEntry *entry,
//                                      const char *name);

// void tree_accessor_create(TreeAccessor *tree, TreeValueRawGetter rawGetter,
//                           TreeValueGetter getter, TreeValueSetter setter) {
//     tree->root.name = "/";
...
// bool tree_accessor_item_register(TreeAccessor *tree, const char *containerPath,
...
//         return false;
//     }
//     TreeAccessorItemEntry *container =
//         tree->context._parsedNodes[tree->context._currentParsedNodeIndex]
//             .currentEntry;

//     if (tree_child_find((Tree *)container, entry->name,
//                         (CompareFunction)&_tree_accessor_match_name) != NULL) {
...
// bool tree_accessor_value_set(TreeAccessor *tree, const char *path,
//                              char *value) {
//     if (!_tree_accessor_path_parse(tree, path)) {
//         return false;
//     }
...
// bool tree_accessor_value_get(TreeAccessor *tree, const char *path,
//                              char *outBuffer) {
//     if (!_tree_accessor_path_parse(tree, path)) {
//         return false;
//     }
...
//         }
//         strlcat(ctx->path, ctx->contextNodes[i].currentEntry->name,
//                 CONSOLE_PATH_MAX_SIZE);
...
//         return NULL;
//     }
//     TreeAccessorPathNode *node =
//         &tree->context._parsedNodes[tree->context._currentParsedNodeIndex];
//     TreeAccessorItemEntry *child =
//         (TreeAccessorItemEntry *)node->currentEntry->base.child;
...
//             TreeAccessorPathContextNode *cNodeParent =
//                 &ctx->contextNodes[i - 1];
...
//             } else {
//                 cNodeParent->currentEntry->rawGetter(cNodeParent->parentData,
//                                                      cNodeParent->index,
//                                                      &cNode->parentData);
//             }
//         }
...
//     } else {
//         void *parentData;
//         TreeAccessorPathContextNode *parentNode =
//             &ctx->contextNodes[ctx->_parsedNodeBackTrace];
//         if (parentNode->currentEntry->rawGetter == NULL) {
//             parentData = NULL;
//         } else {
//             parentNode->currentEntry->rawGetter(parentNode->parentData,
//                                                 parentNode->index, &parentData);
//         }
...
//                 pData = NULL;
//             } else {
//                 pNode->currentEntry->rawGetter(parentData, pNode->index,
//                                                &pData);
//             }
//             parentData = pData;
//         }
...
//             ctx->_parsedNodes[i].currentEntry =
//                 ctx->contextNodes[i].currentEntry;
...
//         ctx->_parsedNodeBackTrace =
//             min(ctx->_parsedNodeBackTrace, ctx->_currentParsedNodeIndex);
//         return true;
...
//         TreeAccessorItemEntry *curEntry = _tree_accessor_entry_child_find(
//             parentNode->currentEntry, identity, identitySize);
//         if (curEntry == NULL) {
