...
#define next_field()                            \
    ...
    while (*format)
    {
        ...
        case 'f':
        { // Fractional value with scale (struct NmeaFloat).
            int sign = 0;
...
bool_t nmea_sentence_entry_get(NmeaParser *parser, const char *sentence, bool_t strict, NmeaSentenceEntry **result)
{
    ...
    {
        NmeaSentenceEntry *te = parser->entries[i];
        if (te == NULL)
        {
            return false;
        }
        if (memcmp(sentence, te->talkerStr, 2) &&
            memcmp(sentence + 2, te->idStr, 3))
        {
            *result = te;
            return true;
        }
...
bool_t nmea_sentence_register(NmeaParser *parser, NmeaSentenceEntry *entry)
{
    ...
    {
        if (parser->entries[i] == NULL)
        {
            parser->entries[i] = entry;
            return true;
        }
    }
    return false;
