 ...
 */
static inline int_least32_t nmea_rescale(struct NmeaFloat *f,
 ...
 */
bool NmeaParser::sentence_entry_get(const char *sentence, bool strict,
...
bool NmeaSentenceBase::match(const char *sentence) {
    return memcmp(sentence, this->talkerStr, 2) &&
           memcmp(sentence + 2, this->idStr, 3);
};
...
bool NmeaSentenceRMC::parse(void *frame, const char *sentence) {
    ...
    if (!Sentence::scan(sentence, "tTcfdfdffDfd", type, &data->time, &validity,
                        &data->latitude, &latitude_direction, &data->longitude,
                        &longitude_direction, &data->speed, &data->course,
                        &data->date, &data->variation, &variation_direction))
        return false;
...
bool NmeaSentenceGGA::parse(void *frame, const char *sentence) {
    ...
    if (!Sentence::scan(sentence, "tTfdfdiiffcfcf_", type, &data->time,
                        &data->latitude, &latitude_direction, &data->longitude,
                        &longitude_direction, &data->fix_quality,
                        &data->satellites_tracked, &data->hdop, &data->altitude,
                        &data->altitude_units, &data->height,
                        &data->height_units, &data->dgps_age))
        return false;
...
bool NmeaSentenceGSA::parse(void *frame, const char *sentence) {
    ...
    if (!Sentence::scan(sentence, "tciiiiiiiiiiiiifff", type, &data->mode,
                        &data->fix_type, &data->sats[0], &data->sats[1],
                        &data->sats[2], &data->sats[3], &data->sats[4],
                        &data->sats[5], &data->sats[6], &data->sats[7],
                        &data->sats[8], &data->sats[9], &data->sats[10],
                        &data->sats[11], &data->pdop, &data->hdop, &data->vdop))
        return false;
...
bool NmeaSentenceGLL::parse(void *frame, const char *sentence) {
    ...
    if (!Sentence::scan(sentence, "tfdfdTc;c", type, &data->latitude,
                        &latitude_direction, &data->longitude,
                        &longitude_direction, &data->time, &data->status,
                        &data->mode))
        return false;
...
bool NmeaSentenceGST::parse(void *frame, const char *sentence) {
    ...
    if (!Sentence::scan(
            sentence, "tTfffffff", type, &data->time, &data->rms_deviation,
...
bool NmeaSentenceGSV::parse(void *frame, const char *sentence) {
    ...
    if (!Sentence::scan(
            sentence, "tiii;iiiiiiiiiiiiiiii", type, &data->total_msgs,
            &data->msg_nr, &data->total_sats, &data->sats[0].nr,
            &data->sats[0].elevation, &data->sats[0].azimuth,
            &data->sats[0].snr, &data->sats[1].nr, &data->sats[1].elevation,
            &data->sats[1].azimuth, &data->sats[1].snr, &data->sats[2].nr,
            &data->sats[2].elevation, &data->sats[2].azimuth,
...
bool NmeaSentenceVTG::parse(void *frame, const char *sentence) {
    ...
    if (!Sentence::scan(
            sentence, "tfcfcfcfc;c", type, &data->true_track_degrees, &c_true,
            &data->magnetic_track_degrees, &c_magnetic, &data->speed_knots,
            &c_knots, &data->speed_kph, &c_kph, &c_faa_mode))
        return false;
...
bool NmeaSentenceZDA::parse(void *frame, const char *sentence) {
    ...
    if (!Sentence::scan(sentence, "tTiiiii", type, &data->time, &data->date.day,
                        &data->date.month, &data->date.year, &data->hour_offset,
                        &data->minute_offset))
        return false;
    ...
    // check offsets
    if (abs(data->hour_offset) > 13 || data->minute_offset > 59 ||
        data->minute_offset < 0)
        return false;
